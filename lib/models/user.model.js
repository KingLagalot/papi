...
class User {
    static get_fields = ['id', 'display_name', 'first_name', 'last_name', 'photo_url', 'email', 'username', 'account_type'];
    ...
    static async create(fields, ret_fields){
        ...
        const user = (await db('users').insert(fields, ret_fields ? this.get_fields.concat(ret_fields) : this.get_fields))[0];
        return user;
        }catch(err){
            console.error(err);
        }
