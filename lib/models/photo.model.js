...
class Photo {
  ...
  async moveAndSize(file) {
    Jimp.read(file.path, (err, jp) => {
      if (err) throw err;
      jp.resize(256, 256).write(
        `${process.env.STORAGE_DIR}/${this.author_id}/tiny/${this.id}.png`,
      ); // save
      jp.resize(512, 512).write(
        `${process.env.STORAGE_DIR}/${this.author_id}/small/${this.id}.png`,
      ); // save
      jp.resize(1024, 1024).write(
        `${process.env.STORAGE_DIR}/${this.author_id}/medium/${this.id}.png`,
      ); // save
      jp.resize(2048, 2048).write(
        `${process.env.STORAGE_DIR}/${this.author_id}/large/${this.id}.png`,
      ); // save
    });
  }
